...
{if $myads != 0}
	...
	{section name=c loop=$adcategory}
        ...
        {if $adnumber > 0}
            ...
            {foreach item=ad from=$advertisement}
            	{if $ad.category == $adcategory[c].id}
                	{if $smarty.session.adSync == $ad.token}<div style="display:none">{/if}
            ...
            {/if}
                 <td valign="top">{$ad.descr}</td>
                 </tr>
                 </table>
                        </div>
                        {/if}
                        <div class="ad-footer">
                        Earn {set_amount($ad.value)}
                        </div>
                </div> 
                	{if $smarty.session.adSync == $ad.token}</div>{/if}
                {/if}
            {/foreach} 
            <div class="clear"></div>
        {/if}
