...
function set_gateway(val){
	if(val != ''){
		if(val == 'balance'){
			$("#min_deposit").html('{$lang.txt.min_deposit}: {set_amount()}{$settings.amount_transfer}');
		}else{
			$("#min_deposit").html('{$lang.txt.min_deposit}: {set_amount()}'+gateway[val]);
		}
		$("#min_deposit").show();
	}else{
		$("#min_deposit").hide();
...
function complete_deposit(){
	...
	if(isNaN(parseFloat(amount))){
			$("#error_box").html('{$lang.txt.min_deposit}: {set_amount()}{$settings.amount_transfer}');
			$("#error_box").fadeIn();
			return false;
	}
	...
	if(gatewayid == 'balance'){
		...
		if(amount < gateway[gatewayid]){
			$("#error_box").html('{$lang.txt.min_deposit}: {set_amount()}'+gateway[gatewayid]);
			$("#error_box").fadeIn();
		}else{
	...
	<table width="100%" class="widget-tbl" id="addfrm">
     ...
     {if $settings.money_transfer == 'yes'}
     	<div id="acc_balancefrm" style="display:none" class="gatewayfrm">
            ...
            <input type="image" src="{base_url('assets/evolution/images/gateway_deposit/ab.png')}" width="100" />
