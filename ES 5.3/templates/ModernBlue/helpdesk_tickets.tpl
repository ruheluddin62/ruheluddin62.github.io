...
<table width="100%" class="widget-tbl">
	...
	<tr>
        ...
        <td>{$ticket_info.date|date_format:"%A, %B %e, %Y at %I:%M %p"}</td>
    </tr>
	<tr>
...
<fieldset class="ticket-user">
	<legend>{$lang.txt.you}, {$ticket_info.date|date_format:"%A, %B %e, %Y at %I:%M %p"}</legend>
    {$ticket_info.message}
</fieldset>    
...
<fieldset class="ticket-user">  
  <legend>{$lang.txt.you}, {$reply_info[r].date|date_format:"%A, %B %e, %Y at %I:%M %p"}</legend>
	{else}
<fieldset class="ticket-admin">  
  <legend><strong>{$lang.txt.administrator}, {$reply_info[r].date|date_format:"%A, %B %e, %Y at %I:%M %p"}</strong></legend>
    {/if}
...
<div align="center" style="padding-top:10px; padding-bottom:5px;">
    ...
    <input type="button" name="btn" value="Go back" onclick="history.back();" />
</div>
</form>
{/if}
