...
<table width="100%" class="frm-tbl">   
        {foreach item=item from=$thelist}
            <tr>
                ...
                <img src="{base_url('assets/evolution/images/forum/forum.png')}" />
                </td>
                <td>
                <div class="frm_title">
                {if $item.sticky == 1}
                <img src="{base_url('assets/evolution/images/forum/t_announcement.png')}" />
                {/if}
                {if $item.locked == 1}
                <img src="{base_url('assets/evolution/images/forum/t_locked.png')}" />
                {/if}
            ...
            {/if}
                ...
                {elseif $item.last_post_date|date_format == $yesterday|date_format}
                    {$lang.txt.yesterdayat} {$item.date|date_format:"%H:%M"}
                {else}
                    {$item.last_post_date|date_format:"%Y-%m-%d at %H:%M"}
                {/if}
                </div>
                </td>
            </tr>
